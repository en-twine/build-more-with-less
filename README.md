...
The default guards allow six normal provider requests per user task and twelve across the complete session, including delegated subagents. When either request ceiling is reached, `reserveFinalRequest: true` permits exactly one additional provider request. During that request the normal application tools are removed, so Pi can only return a completion message when the criteria were already satisfied or call the dedicated `save_handoff` tool. That tool deterministically overwrites the single local `.pi-handoff.md` with its sender/model, timestamp, workspace, objective, completed and remaining work, verification state, and exact next step. The file lives in this harness repository, has user-only file permissions, is git-ignored, is never added to model context automatically, and is never shared. Its absolute path is shown locally when it is updated.
